...
def dodaj_wszystko():
    tekst_kamera = entry_name.get()
    tekst_pracownik = entry_surname.get()
    tekst_konserwator = entry_location.get()

    if tekst_kamera.strip():
        listbox_lista_kamer.insert(END, tekst_kamera)
        entry_name.delete(0, END)

    if tekst_pracownik.strip():
        listbox_lista_pracownikow.insert(END, tekst_pracownik)
        entry_surname.delete(0, END)

    if tekst_konserwator.strip():
        listbox_lista_konserwatorow.insert(END, tekst_konserwator)
...
Button(ramka_formularz, text="Dodaj", command=dodaj_wszystko).grid(row=4, column=0, columnspan=2)
